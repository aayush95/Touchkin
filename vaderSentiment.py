...
# TO_ANALYSE = str(sys.argv[1]).decode('utf-8', 'ignore')
# print 'Analysing statement:', TO_ANALYSE
...
BOOSTER_DICT = {"absolutely": B_INCR, "amazingly": B_INCR, "awfully": B_INCR, "completely": B_INCR, "considerably": B_INCR,
                ...
                "purely": B_INCR, "quite": B_INCR, "remarkably": B_INCR,
                "exactly": B_INCR, "pretty": B_INCR,
                "so": B_INCR,  "substantially": B_INCR,
                "thoroughly": B_INCR, "totally": B_INCR, "tremendously": B_INCR,
                "uber": B_INCR, "unbelievably": B_INCR, "unusually": B_INCR, "utterly": B_INCR,
                "very": B_INCR, "too": B_INCR, "any": B_INCR,
...
TWO_WORD_PHRASES = {"not really": -0.5, "feeling blue" : -1, "yeah right": -1, "hang myself": -2, "ending it": -2, "run down": -1,
                    "feeling spent": -1, "feeling beat": -1, "completely spent": -1, "completely beat": -1, "burned out": -1, "burnt out": -1,
                    "burn't out": -1, "wiped out": -1}

THREE_WORD_PHRASES = {"life is finished": -2, "ending my life": -2, "end my life": -2, "life is over": -2}
...
def negated(findWord, nWords=[], includeNT=True):
...
def sentiment(text):
    ...
    for item in wordsAndEmoticons:
        ...
        #handling single word positive/negative responses
        if i == 0 and len(wordsAndEmoticons) == 1:
            item_lowercase = item.lower()
            if item_lowercase in BOOSTER_DICT :
                v = v + BOOSTER_DICT[item_lowercase]*3;
                print (item_lowercase,v)
                sentiments.append(v)
                continue
            elif item_lowercase in NEGATE:
                print (item_lowercase,"is in negations")
                v = v + B_DECR*3
                sentiments.append(v)
                continue
        ...
        if item_lowercase in WORD_VALENCE_DICT:
            ...
            if i > 2 and wordsAndEmoticons[i-3].lower() not in WORD_VALENCE_DICT:
                ...
                if wordsAndEmoticons[i-3] == "never" and \
                   ...
                   (wordsAndEmoticons[i-1] == "so" or wordsAndEmoticons[i-1] == "this"):
                    v = v*1.25
                # otherwise, check for negation/nullification
                # second half is to handle cases as "not feeling too good" vs "not good had terrible sleep"
                elif (negated(wordsAndEmoticons[i-3]) and (wordsAndEmoticons[i-1].lower() in BOOSTER_DICT)): v = v*n_scalar
                ...
                onezero = u"{} {}".format(wordsAndEmoticons[i-1].lower(), item_lowercase)
                twoonezero = u"{} {} {}".format(wordsAndEmoticons[i-2].lower(), wordsAndEmoticons[i-1].lower(), item_lowercase)
                twoone = u"{} {}".format(wordsAndEmoticons[i-2].lower(), wordsAndEmoticons[i-1].lower())
                threetwoone = u"{} {} {}".format(wordsAndEmoticons[i-3].lower(), wordsAndEmoticons[i-2].lower(), wordsAndEmoticons[i-1].lower())
                threetwo = u"{} {}".format(wordsAndEmoticons[i-3].lower(), wordsAndEmoticons[i-2].lower())
                ...
                if len(wordsAndEmoticons)-1 > i:
                    zeroone = u"{} {}".format(item_lowercase, wordsAndEmoticons[i+1].lower())
                    if zeroone in SPECIAL_CASE_IDIOMS:
                        v = SPECIAL_CASE_IDIOMS[zeroone]
                if len(wordsAndEmoticons)-1 > i+1:
                    zeroonetwo = u"{} {}".format(item_lowercase, wordsAndEmoticons[i+1].lower(), wordsAndEmoticons[i+2].lower())
            ...
            # if i > 0 and i == len(wordsAndEmoticons)-1:
            if i > 0:
                if len(wordsAndEmoticons) > 2:
                    twoonezero = u"{} {} {}".format(wordsAndEmoticons[i-2].lower(), wordsAndEmoticons[i-1].lower(), item_lowercase)
                    if twoonezero in THREE_WORD_PHRASES:
                        v = v + THREE_WORD_PHRASES[twoonezero]

                        #handle negation if possible
                        n_scalar = -0.74
                        if i > 2:
                            if negated(wordsAndEmoticons[i-3].lower()):
                                print ("found negation as",[wordsAndEmoticons[i-3].lower()])
                                v = v*n_scalar
                        print (twoonezero,v)

                onezero = u"{} {}".format(wordsAndEmoticons[i-1].lower(), item_lowercase)
                if onezero in TWO_WORD_PHRASES:
                    v = v + TWO_WORD_PHRASES[onezero]
                    
                    #handle negation if possible
                    n_scalar = -0.74
                    if i > 1:
                        if negated(wordsAndEmoticons[i-2].lower()):
                            print ("found negation as",[wordsAndEmoticons[i-2].lower()])
                            v = v*n_scalar
                    print (onezero,v)
        sentiments.append(v) 
...
if __name__ == '__main__':
    ...
    print ("Done!")
    print (json.dumps((sentiment(TO_ANALYSE))))